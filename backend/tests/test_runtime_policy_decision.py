...
def test_darwin_with_unknown_vram_forces_api() -> None:
    assert decide_force_api_generations(system="Darwin", cuda_available=False, vram_gb=None) is True


def test_darwin_with_low_vram_forces_api() -> None:
    assert decide_force_api_generations(system="Darwin", cuda_available=False, vram_gb=14) is True


def test_darwin_with_required_vram_allows_local_mode() -> None:
    assert decide_force_api_generations(system="Darwin", cuda_available=False, vram_gb=15) is False
    assert decide_force_api_generations(system="Darwin", cuda_available=False, vram_gb=24) is False
    assert decide_force_api_generations(system="Darwin", cuda_available=False, vram_gb=48) is False
