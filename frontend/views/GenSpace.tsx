  ...
  const handleGenerate = async () => {
    ...
    for (let i = totalBatch; i >= 1; i--) {
      ...
      if (mode === 'image') {
        ...
        // generation clears videoUrl from state. Mark key so the useEffect skips it.
        if (result.videoUrl && result.videoPath && currentProjectId) {
          const generationKey = `${result.videoUrl}|${result.videoPath}`
          persistedVideoKeyRef.current = generationKey
          ...
          const savedVideoSettings = applyForcedVideoSettings(settings)
          try {
            const copied = await copyToAssetFolder(result.videoPath, currentProjectId)
            const finalPath = copied?.path ?? result.videoPath
            const finalUrl = copied?.url ?? result.videoUrl
            addAsset(currentProjectId, {
              type: 'video',
              path: finalPath,
              url: finalUrl,
